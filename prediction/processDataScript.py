...
class PredictPD:
    ...
    def __init__(self):
        ...
        # -------- Initialize features --------
        self.countAvgPassesFeature = processData.CountAvgPassesFeature(countAvgFile)
        self.playerPosFeature = processData.PlayerPositionFeature(squad_dir)
        self.rankFeature = processData.RankingFeature(rankFile)
        self.meanDegreeFeature = processData.MeanDegreeFeature()
        self.betweenFeature = processData.BetweennessFeature()
        self.passComplAttempFeature = processData.PassesComplAttempPerPlayerFeature()
        self.countPassesPosFeature = processData.CountPassesPerPosFeature(passPerPosDir, "group")
        self.passComplAttempTeamFeature = processData.CountPassesComplAttempPerTeamFeature("group")
        # -------------------------------------
...
ls
            ...
            for game in allGames:
                ...
                for players in edgeFile:
                    ...
                    # print "weights[%s][%s] = %f" %features["p1"] = p1
                    features["p1"] = p1
                    features["p2"] = p2
                    features["weight"] = weight
                    features["diffInRank"] = 1 if features["diffInRank"] else 0
